-- HallyuHub account deletion v3.1
-- Proposal only. Do not execute before review.
-- This migration never deletes auth.users or user content.
...
begin
  if to_regclass('public.account_deletion_requests') is not null then
    raise exception 'account_deletion_requests already exists; review previous migrations';
  end if;
  if to_regprocedure('public.hallyu_request_account_deletion(text,boolean)') is not null then
    raise exception 'request RPC already exists; review previous migrations';
  end if;
  if to_regprocedure('public.hallyu_get_account_deletion_status()') is not null then
    raise exception 'status RPC already exists; review previous migrations';
  end if;
  if to_regprocedure('public.hallyu_cancel_account_deletion(uuid)') is not null then
    raise exception 'cancel RPC already exists; review previous migrations';
  end if;
