-- HallyuHub Beta Real - Home feed con usuarios seguidos y entidades K-pop seguidas.
-- Version segura: no crea ni altera public.kpop_entities.
...
do $$
begin
  if to_regclass('public.kpop_entities') is null then
    raise exception 'Falta public.kpop_entities. Corré primero la migración del catálogo K-pop antes de esta migración de Inicio/feed.';
  end if;

  if to_regprocedure('public.can_view_profile_content(uuid)') is null then
    raise exception 'Falta public.can_view_profile_content(uuid). Corré primero la migración de privacidad de perfiles.';
  end if;
end $$;
